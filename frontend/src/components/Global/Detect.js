...
const Detect = () => {
  ...
  const [stringGame, setStringGame] = useState(
    "person,person,bicycle,person,mouse,car,mouse,car,car"
  );
  ...
  const uploadHandler = async () => {
    ...
    try {
      ...
      const test = await axios.get(`${process.env.REACT_APP_API}/test`);
      await setImagePrediction(res.data);
      await setStringGame(test.data); //// NOI DO API tra ve string
  ...
  return (
    <div className="detection">
      <div className="detection__upload">
        ...
        {imagePrediction && (
          <div className="detect__result-checkbox">
            <div className="checkbox-text">
              {!isHide ? "Show result" : "Hide result"}{" "}
            </div>
            <input
              type="checkbox"
        ...
        {imagePrediction && (
          <>
            <div className="detection__your-predict-title" data-aos="fade-down">
              Our model can detect {arrGame.length} objects. Can you guess it?
            </div>
            <ul className="menu_game" data-aos="fade-down">
              <li
                className={
                  kindeLevel === 1
                    ? "menu__game-item--active"
                    : "menu__game-item"
                }
                onClick={() => setKindLevel(1)}
              >
                Level 1
              </li>
              <li
            ...
            <div className="detection__your-predict" data-aos="fade-down">
              <div className="form__action">
                {kindeLevel === 2 && (
                  <Level2 arrGame={arrGame} previewImageUrl={previewImageUrl} />
                )}
                {kindeLevel === 1 && <Level1 arrGame={arrGame} />}
                {kindeLevel === 3 && (
                  <Level3 arrGame={arrGame} previewImageUrl={previewImageUrl} />
                )}
              </div>
            </div>
          </>
